"""
split_v1 — Stratified patient-level train/val/test splits.
...
They are included as training data but excluded from the White vs Black fairness
comparison. Their stratum is recorded as "Other".
...
def create_splits(df: pl.DataFrame, seed: int | None = None) -> pl.DataFrame:
    """Create stratified patient-level train/val/test splits (race x age).
    ...
    patients = (
        ...
        .select(Col.PATIENT_ID, "race_bin", "age_bin", "stratum")
...
if __name__ == "__main__":
    ...
    logger.info("\nSplit balance (race_bin x age_bin):")
